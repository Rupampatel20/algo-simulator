...
export const SelectionVisualize = () => {
  ...
  const [description, setDescription] = useState("Press 'Start Selection Sort' to begin visualization.");
  ...
  const selectionSort = async () => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      setDescription(`Starting comparison for index ${i}.`);
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        setDescription(`Comparing ${arr[i]} (index ${i}) with ${arr[j]} (index ${j}).`);

        if (arr[j] < arr[minIndex]) {
          minIndex = j;
          setDescription(`Found new minimum value ${arr[minIndex]} (index ${minIndex}).`);
        }
      ...
      if (minIndex !== i) {
        setDescription(`Swapping ${arr[i]} (index ${i}) with ${arr[minIndex]} (index ${minIndex}).`);
      ...
      setDescription(`Placed ${arr[i]} (index ${i}) in its correct position.`);
    }
